...
class TargetImportView(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request):
        ...
        ## ========= OLD - IGNORE UNLESS THINGS FALL APART =========

        # with open(os.getcwd()+"/Data/processed_file.csv", "rb") as f:
        #     lines = f.readlines()
        # print(lines)
        # lines_2 = "".join(str(x)[2:-1] for x in lines)
        # print(lines_2)

        # print(open(os.getcwd()+"/Data/processed_file.csv", "rb").read().decode('utf-8'))
        # # csv_stream = StringIO(open(os.getcwd()+"/Data/processed_file.csv", "rb").read().decode('utf-8'), newline=None)
        # csv_stream = StringIO(lines_2, newline=None)
        # # processed_file.to_csv(csv_stream)
        # result = import_targets(csv_stream)
        # print(csv_stream)
        # print(result)
        # # print(csv_stream)
